...
int main( void )
{
  ...
  int fastest_time;         // 上記で過去の一番早いタイム
  ...
  /* 本体前の準備 */
  for(i=0; i<BULLET; i++) { // 配列の初期化、バグが起こらない値にする
    left[i] = -1;
    right[i] = -1;
    up[i] = -1;
    down[i] = -1;
  ...
  while(1) {   
    ...
    if( key != EOF && (key == 'a' || key == 's' || key == 'd' || key == 'w') ) { // a,s,d,wいづれかの文字を次の入力時まで保存、他のキーなら保存しない
    ...
    // 弾を撃つ操作
    if(key == 'h' && counter < BULLET) { // 撃った数が最初に所持している弾の数と同じになると、それ以降は撃てない
    ...
    for( i=0; i<counter; i++ ) { // 撃った弾を動かし続ける      
      if( right[i] == i ) {       // 右
        draw_point(b[i], ' ');
	b[i].x = b[i].x + B_SPEED;
	draw_point(b[i], '.');
    ...
    dis_x = fabs(p.x-q.x);  // ＸとＡのx座標の差の絶対値
    dis_y = fabs(p.y-q.y);  // ＸとＡのy座標の差の絶対値
    if( dis_x <= 30 && dis_y <= 15 && rand()%2 == 0 ) { // 索敵内ならＸを追いかける
      if( p.x == q.x && p.y < q.y ) {       // Ｘが上にいるとき
	q.y = q.y - REVEL;
      }else if( p.x == q.x && p.y > q.y ) { // Ｘが下にいるとき
	q.y = q.y + REVEL;
      }else if( p.x < q.x && p.y == q.y ) { // Ｘが左にいるとき
	q.x = q.x - REVEL;
      }else if( p.x > q.x && p.y == q.y ) { // Ｘが右にいるとき
	q.x = q.x + REVEL;
      }else if( p.x < q.x && p.y < q.y ) {  // Ｘが左上にいるとき
	if( rand()%2 == 0 ) { //上か左どちらかに移動すれば近付く
	  q.x = q.x - REVEL;
	}else {
	  q.y = q.y - REVEL;
	}
      }else if( p.x > q.x && p.y < q.y ) {  // Ｘが右上にいるとき
        if( rand()%2 == 0 ) { //上か右どちらかに移動すれば近付く
	  q.x = q.x + REVEL;
	}else {
	  q.y = q.y - REVEL;
	}
      }else if( p.x < q.x && p.y > q.y ) {  // Ｘが左下にいるとき
        if( rand()%2 == 0 ) { //下か左どちらかに移動すれば近付く
	  q.x = q.x - REVEL;
	}else {
	  q.y = q.y + REVEL;
	}
      }else if( p.x > q.x && p.y > q.y ) {  // Ｘが右下にいるとき
        if( rand()%2 == 0 ) { //下か右どちらかに移動すれば近付く
	  q.x = q.x + REVEL;
	}else {
	  q.y = q.y + REVEL;
	}
      }
    }else if( rand()%2 == 0 ) { // 索敵外ならランダムに動く
      if(rand()%4 == 0) { // ランダムに上下左右に動く
        q.x = q.x + REVEL;
      }else if(rand()%4 == 1) {
        q.x = q.x - REVEL;
      }else if(rand()%4 == 2) {
    ...
    // 弾がAに当たったときの処理
    for( i=0; i<counter; i++ ) {
      if(status = check_in_out(b[i]) == 1) { // 弾が画面内であるか判定、そうでなければ最初に戻る
	continue;
      }else if( getchar_point(b[i]) == 'A' ) { // 弾の移動先にAがあるか判定
	...
	if( attack_counter < 5 ) {
	  ...
	  t2 = time(NULL);  // タイム測定終了
          ...
          printw("Game Clear!");          // XをAにぶつけることができたのでゲームクリア
          refresh();
          sleep(wait_last);                 // 少し待つ
          endwin();                         /* 画面制御の終了 */
	  current_time = t2-t1;             // タイム計算
	  fp = fopen("time.dat", "r");      // ファイルを開く
	  fscanf(fp, "%d", &fastest_time); // ファイル読み込み
	  fclose(fp);
	  if( current_time < fastest_time ) { // 過去のタイムよりも速ければ更新
    ...
    // 敵に攻撃を与えられたときの処理
    if( getchar_point(q) == 'X' ) { // Aの移動先にXがあると停止
	++damage_counter;
	if( damage_counter < 3 ) {
	  flash();
	} else{
          ...
          printw("Game over!");         // XをAにぶつけることができたのでゲームクリア
          refresh();
          sleep(wait_last);             // 少し待つ
          endwin();                     /* 画面制御の終了 */
	  return 0;
